...
const businessDiff = function(param) {
  param = moment(param);
  ...
  if (start_offset == 7) start_offset = 5;
  else if (start_offset == 1) start_offset = 0;
  else start_offset -= 2;

  if (end_offset == 6) end_offset--;

  return signal * (weeks * 5 + start_offset + end_offset);
};

const businessAdd = function(days) {
...
const businessSubtract = function(days) {
